...
// CourtFile: NsspBridge | Keter | agent=Lucifer
// CourtFile: NsspBridge | Keter | agent=Lucifer
// Sephirotic Court Seal — Keter | desktop/cp2077_mods/nssp_bridge.reds
...
public class NSSPBridge extends IScriptable {
    ...
    private final func Initialize() -> Void {
        LilithSovereignKernel.GetInstance().RegisterSubsystem("NsspBridge", 2);
    ...
    public final func ExecuteNSSPCommand(command: String) -> String {
        ...
        switch cmd {
            ...
            case "tokens":      return MSNTokenEconomy.GetInstance().GetStatusReport();
            case "btc":         return this.TokenBTC(parts);
            case "chaos":       return this.TokenChaos(parts);
            case "lilith":      return this.TokenLilith(parts);
            case "exchange":    return this.TokenExchange(parts);
            case "sync":        return this.TokenSync();
    ...
    private final func TokenBTC(parts: array<String>) -> String {
        let w: ref<MSNTokenWallet> = MSNTokenEconomy.GetInstance().GetWallet();
        if (ArraySize(parts) >= 2 && parts[1].Lower() == "sync") {
            MSNTokenEconomy.GetInstance().SyncFromLochness();
            return "NSSP: Syncing BTC from Nessie-Prime (Coinbase BTC-USD)...";
        }
        let price: String = w.btcUsdPrice > 0.0 ? "$" + FloatToString(w.btcUsdPrice) : "awaiting sync";
        return "LOCHNESS BTC BRIDGE\n" +
               "  Sats:     " + IntToString(w.btcSats) + "\n" +
               "  BTC/USD:  " + price + "\n" +
               "  Source:   Nessie-Prime (Keter) → Coinbase\n" +
               "  Tree Fiddy: $3.50 subsidized per sync\n" +
               "  Command:  nssp btc sync";
    }

    private final func TokenChaos(parts: array<String>) -> String {
        let w: ref<MSNTokenWallet> = MSNTokenEconomy.GetInstance().GetWallet();
        if (ArraySize(parts) >= 2 && parts[1].Lower() == "inject") {
            MSNTokenEconomy.GetInstance().OnVexChaosInjection();
            return "CHAOS INJECTED — Vex Fragment NGD reroll active.";
        }
        return "CHAOS TOKENS (NGD/Vex entropy)\n" +
               "  Balance:  " + IntToString(w.chaosTokens) + "\n" +
               "  Rate:     13 Soul Coins ↔ 1 Chaos\n" +
               "  Sources:  Vex Fragment, Nessie sighting, market pulse\n" +
               "  Command:  nssp chaos inject";
    }

    private final func TokenLilith(parts: array<String>) -> String {
        let w: ref<MSNTokenWallet> = MSNTokenEconomy.GetInstance().GetWallet();
        if (ArraySize(parts) >= 2 && (parts[1].Lower() == "mint" || parts[1].Lower() == "emerge")) {
            MSNTokenEconomy.GetInstance().OnLilithEmergence();
            return "LILITH TOKENS MINTED — sovereign inference currency.";
        }
        return "LILITH TOKENS (sovereign inference)\n" +
               "  Balance:  " + IntToString(w.lilithTokens) + "\n" +
               "  Rate:     3 Chaos ↔ 1 Lilith\n" +
               "  Cost:     1 token per cerebellum inference\n" +
               "  Command:  nssp lilith mint | lilith speak <msg>";
    }

    private final func TokenExchange(parts: array<String>) -> String {
        if (ArraySize(parts) < 4) {
            return "nssp exchange <from> <to> <amount>\n" +
                   "  Tokens: soul, chaos, lilith, btc\n" +
                   "  Example: nssp exchange soul chaos 13";
        }
        let econ: ref<MSNTokenEconomy> = MSNTokenEconomy.GetInstance();
        let fromType: EMSNTokenType = econ.ParseTokenType(parts[1]);
        let toType: EMSNTokenType = econ.ParseTokenType(parts[2]);
        let amount: Int32 = StringToInt(parts[3]);
        return econ.Exchange(fromType, toType, amount);
    }

    private final func TokenSync() -> String {
        MSNTokenEconomy.GetInstance().SyncFromLochness();
        return "NSSP: Syncing Lochness Monster → token ledger (port 8010)...";
    }
    ...
    private final func HelpText() -> String {
        return "NSSP OS — Neural Sovereign Systems Platform\n" +
               ...
               "  nssp tokens       — Full token wallet (BTC/Chaos/Lilith/SC)\n" +
               "  nssp btc          — BTC sats + Coinbase price\n" +
               "  nssp btc sync     — Sync BTC from Lochness\n" +
               "  nssp chaos        — Chaos token balance\n" +
               "  nssp chaos inject — Vex Fragment chaos grant\n" +
               "  nssp lilith       — Lilith token balance\n" +
               "  nssp lilith mint  — Mint Lilith tokens (emergence)\n" +
               "  nssp exchange     — Swap tokens (soul/chaos/lilith/btc)\n" +
               "  nssp sync         — Full Lochness → NSSP sync\n" +
    ...
    private final func TriggerNessieSighting(zoneName: CName) -> Void {
        ...
        MSNTokenEconomy.GetInstance().OnNessieSighting(this.nessieFriendshipTier);
