...
// Sephirotic Court Seal — Tiferet | desktop/cp2077_mods/msn_sephirotic_npcs.reds
...
public class MSNSephiroticCourt extends IScriptable {
    ...
    private let courtFileCounts: array<Int32>;
    private let courtFilesTotal: Int32;
    ...
    public final func Initialize() -> Void {
        LilithSovereignKernel.GetInstance().RegisterSubsystem("MsnSephiroticNpcs", 2);
    ...
    public final func RegisterCourtBatch(sephirah: CName, count: Int32, agent: CName) -> Void {
        this.courtFilesTotal += count;
        this.courtFileCounts.PushBack(count);
        LogInfo("[Sephirotic Court] Batch " + ToString(sephirah) + " x" + IntToString(count) + " via " + ToString(agent));
    }

    public final func GetCourtFileCount() -> Int32 {
        return this.courtFilesTotal;
    }

    public final func BuildStatusLine() -> String {
        return "MSN Court active=" + ToString(this.activeAgent) +
               " agents=" + ToString(ArraySize(this.agents)) +
               " court_files=" + IntToString(this.courtFilesTotal);
    }
}
