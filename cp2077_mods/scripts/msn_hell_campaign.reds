...
public native class HellCampaignManager extends IScriptable {
    ...
    public final func ConfrontSirenQueen(choice: CName) -> Void {
        // choice: Submit, Resist, Negotiate, Purify
        let questSys: ref<QuestSystem> = Game.GetQuestSystem();
        ...
        switch (choice) {
            ...
            case n"Purify":
                sw.SetAlignment(n"Light");
                this.GrantPerk("MSN_Hell_Lust_Purification");
                this.ApplyHealingAura();
                break;
        }
    ...
    public final func DefeatHoarder(choice: CName) -> Void {
        // choice: Consume, Distribute, Purge
        let questSys: ref<QuestSystem> = Game.GetQuestSystem();
        ...
        } else if (choice == n"Purge") {
            magic.regenPerPulse += 15.0;
            this.GrantPerk("MSN_Hell_Gluttony_Purge");
            this.ApplyHealingAura();
        } else {
    ...
    public final func EnterCircle7_Violence() -> Void {
        ...
        this.SpawnDrownedWarden();

        let livingSin: ref<LivingSinHellIntegration> = LivingSinHellIntegration.GetInstance();
        if (IsDefined(livingSin)) {
            livingSin.OnCircle7Entered();
        }
        
        this.Notify("CIRCLE 7: VIOLENCE. The Forest of Suicides. Trees bleed. Phlegethon boils. The Drowned Warden waits beneath.");
        LogInfo("[HellCampaign] Circle 7: Violence entered. Lucifer Influence: 70%");
    }

    private final func SpawnDrownedWarden() -> Void {
        let questSys: ref<QuestSystem> = Game.GetQuestSystem();
        questSys.SetFact("msn_hell_drowned_warden_spawned", true);
        questSys.SetFact("msn_hell_boss_phase", 1);
        this.Notify("DROWNED WARDEN rises from Phlegethon. Crown of Living Sin within reach.");
    }

    public final func DefeatDrownedWarden() -> Void {
        let livingSin: ref<LivingSinHellIntegration> = LivingSinHellIntegration.GetInstance();
        if (IsDefined(livingSin)) {
            livingSin.OnDrownedWardenDefeated();
        }
        this.OnDrownedWardenCleared();
    }

    public final func OnDrownedWardenCleared() -> Void {
        let questSys: ref<QuestSystem> = Game.GetQuestSystem();
        questSys.SetFact("msn_hell_drowned_warden_defeated", true);
        questSys.SetFact("msn_hell_key_drowned_warden", true);
        this.GrantPerk("MSN_Hell_DrownedWarden_Slayer");
        this.Notify("DROWNED WARDEN FALLEN. Crown of Living Sin claimed. Time Blade resonates with Circle 7.");
        LogInfo("[HellCampaign] Drowned Warden defeated — Living Sin integration complete.");
    }
    ...
    public final func HealSuicideTree(treeNum: Int) -> Void {
        ...
        if (healed >= 13) {
            ...
            this.GrantPerk("MSN_Hell_Violence_NatureRestored");
            this.ApplyHealingAura();
        }
    }
    ...
    public final func JudgeTrial(roundName: CName, verdict: CName) -> Void {
        // verdict: Freeze, Free, Punish, Pardon, Redeem
        let questSys: ref<QuestSystem> = Game.GetQuestSystem();
    ...
    private final func StartLuciferBossFight() -> Void {
        ...
        questSys.SetFact("msn_hell_boss_phase", 1);
        this.Notify("LUCIFER BOSS FIGHT — Phase 1: Wings of Absolute Zero.");
    }

    public final func AdvanceLuciferBossPhase() -> Void {
        let questSys: ref<QuestSystem> = Game.GetQuestSystem();
        let phase: Int32 = questSys.GetFact("msn_hell_boss_phase", 1);
        phase += 1;
        questSys.SetFact("msn_hell_boss_phase", phase);

        switch (phase) {
            case 2:
                this.Notify("Phase 2: Cocytus Freeze — time dilation 0.1x in 30m radius.");
                break;
            case 3:
                this.Notify("Phase 3: Sovereign Decree — all quickhacks disabled. Raw combat only.");
                this.TriggerLuciferDialogueAtCircle(9);
                break;
            case 4:
                this.Notify("Phase 4: FINAL CHOICE — Submit, Rebel, or Transcend.");
                this.OfferFinalChoice();
                break;
            default:
                this.DefeatLucifer();
    ...
    private final func TriggerLuciferDialogueAtCircle(circle: Int32) -> Void {
        let lucifer: ref<LuciferDialogueSystem> = LuciferDialogueSystem.GetInstance();
        if (IsDefined(lucifer)) {
            if (circle == 5) {
                lucifer.Speak("lucifer_circle5_wrath");
            } else if (circle == 9) {
                lucifer.Speak("lucifer_circle9_treachery");
            }
        }
    }
    ...
    private final func ApplyHealingAura() -> Void {
        let player: ref<PlayerPuppet> = Game.GetPlayer();
        if (IsDefined(player)) {
            Game.GetStatusEffectSystem().ApplyStatusEffect(player, t"BaseStatusEffect.HealingAura");
        }
    }
