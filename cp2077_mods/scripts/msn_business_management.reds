...
// CourtFile: MsnBusinessManagement | Chesed | agent=Thoth
// CourtFile: MsnBusinessManagement | Chesed | agent=Thoth
// Sephirotic Court Seal — Chesed | desktop/cp2077_mods/msn_business_management.reds
...
public class BusinessManager extends IScriptable {
    ...
    public final static func GetInstance() -> ref<BusinessManager> {
        LilithSovereignKernel.GetInstance().RegisterSubsystem("MsnBusinessManagement", 1);
    ...
    public final func ProcessPayrollEffects(player: ref<PlayerPuppet>) -> Void {
        ...
        if badGuysCount > 0 {
            ...
            let revenueAmount = badGuysCount * 13; // Balanced to harmonize with 13 SC per Chaos
            let revenueSpike = ToString(revenueAmount);
            MSN_Log("Syndicate_Employees", "BAD_GUY_PAYROLL | Extortion Successful | Revenue Spike: +" + revenueSpike + "_SC");
            
            // Actually grant the Soul Coins to harmonize the economy
            MSNTokenEconomy.GetInstance().GrantToken(EMSNTokenType.SOUL_COIN, revenueAmount, "Extortion Successful");
