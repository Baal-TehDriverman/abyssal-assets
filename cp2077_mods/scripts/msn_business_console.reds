...
// CourtFile: MsnBusinessConsole | Chesed | agent=Thoth
// CourtFile: MsnBusinessConsole | Chesed | agent=Thoth
// Sephirotic Court Seal — Chesed | desktop/cp2077_mods/msn_business_console.reds
...
public class MSNBusinessConsole extends IScriptable {
    ...
    public final func OnInitialize() -> Void {
        LilithSovereignKernel.GetInstance().RegisterSubsystem("MsnBusinessConsole", 2);
    ...
    public final func Cmd_SoulCoinsStatus(args: array<String>) -> Void {
        ...
        LogInfo("Soul Coins: earned through dredging, crafting, trading, Nessie sightings, and story progression.");
        LogInfo("Cross-game: Abyssal Assets Loch Exchange ↔ Cyberpunk NSSP ↔ Lochness BTC bridge.");
    }
    ...
    [ConsoleCommand("msn.skill.trees", "List Abyssal skill trees from SkillTreeRegistry")]
    public final func Cmd_MSNSkillTrees(args: array<String>) -> Void {
        let names = SkillTreeRegistry.GetAbyssalTreeNames();
        let i: Int32 = 0;
        while i < ArraySize(names) {
            LogInfo("Tree: " + names[i]);
            i += 1;
        }
        LogInfo("Abyssal: " + IntToString(SkillTreeRegistry.AbyssalSkillCount) + " skills, "
            + IntToString(SkillTreeRegistry.AbyssalTreeCount) + " trees.");
    }

    [ConsoleCommand("msn.skill.status", "Show unified skill tree + MSN weapon mastery status")]
    public final func Cmd_MSNSkillStatus(args: array<String>) -> Void {
        LogInfo("=== MSN Skill Status ===");
        LogInfo("Abyssal skills: " + IntToString(SkillTreeRegistry.AbyssalSkillCount));
        LogInfo("Abyssal trees: " + IntToString(SkillTreeRegistry.AbyssalTreeCount));
        LogInfo("MSN weapon tiers: " + IntToString(SkillTreeRegistry.MSNWeaponTierCount));
        LogInfo("Hermes categories: " + IntToString(SkillTreeRegistry.HermesCategoryCount));
        let tiers = SkillTreeRegistry.GetWeaponTierSummary();
        let t: Int32 = 0;
        while t < ArraySize(tiers) {
            LogInfo(tiers[t]);
            t += 1;
        }
        LogInfo("Console: abyssal.skill.xp | abyssal.skills.trees | msn.court.status");
    }
