...
// CourtFile: MsnMagicSystem | Hod | agent=Hod
// CourtFile: MsnMagicSystem | Hod | agent=Hod
// Sephirotic Court Seal — Hod | desktop/cp2077_mods/msn_magic_system.reds
...
public class MSNMagicSystem extends IScriptable {
    ...
    private final func Initialize() -> Void {
        LilithSovereignKernel.GetInstance().RegisterSubsystem("MsnMagicSystem", 2);
    ...
    private final func ApplySpellEffect(spell: ref<MSNSpellData>) -> Void {
        ...
        switch (spell.effect) {
            ...
            case n"VoidConductor":
                this.ApplyVoidConductor(spell, player);
                break;
        };
    }
    ...
    private final func MeteorSwarm(spell: ref<MSNSpellData>) -> Void {
        LogInfo("[MSNMagic] Meteor Swarm: " + NameToString(spell.name) + " | 4 projectiles | " + FloatToString(spell.radius) + "m radius");
    }

    private final func ApplyVoidConductor(spell: ref<MSNSpellData>, player: ref<PlayerPuppet>) -> Void {
        LogInfo("[MSNMagic] VOID CONDUCTOR ACTIVE: Entity releases all individual energy to become transparent to Nuit.");
        // Instead of damage, we grant infinite capacity (healing + mana resonance + invulnerability frame)
        this.currentMana = this.maxMana;
        let statsID: StatsObjectID = Cast<StatsObjectID>(player.GetEntityID());
        Game.GetStatPoolsSystem().RequestSettingStatPoolValue(statsID, gamedataStatPoolType.Health, 100.0, player);
        Game.GetUIManager().ShowNotification("Void Conductor: Transparency Achieved. Unified Field Resonance.");
    }
    ...
    private final func GetSpellData(name: CName) -> ref<MSNSpellData> {
        ...
        switch (name) {
            ...
            case n"Weird":
                spell.school = n"Illusion"; spell.level = 9; spell.manaCost = 90.0; spell.cooldown = 120.0; spell.duration = 600.0; spell.damage = 100.0; spell.effect = n"PhantasmalKill"; break;

            // VOID RUNNERS / RA-HOOR-KHUIT
            case n"VoidConductor_Spell":
                spell.school = n"Void"; spell.level = 9; spell.manaCost = 0.0; spell.cooldown = 3600.0; spell.effect = n"VoidConductor"; break;
