...
// CourtFile: MsnMagicQuests | Tiferet | agent=Ouroboros
// CourtFile: MsnMagicQuests | Tiferet | agent=Ouroboros
// Sephirotic Court Seal — Tiferet | desktop/cp2077_mods/msn_magic_quests.reds
...
public native class MagicQuestManager extends IScriptable {
    ...
    private final func Initialize() -> Void {
        LilithSovereignKernel.GetInstance().RegisterSubsystem("MsnMagicQuests", 2);
    ...
    public final func CastRitual(spellName: CName, participants: Int) -> Void {
        ...
        switch (spellName) {
            ...
            case n"Gate":
                ...
                break;
            case n"MassHeal":
                this.CastMassHeal(participants);
                break;
            case n"DivineAegis":
                this.CastDivineAegis(participants);
                break;
            default:
    ...
    private final func CastMassHeal(participants: Int) -> Void {
        if (participants < 2) {
            this.Notify("Mass Heal ritual requires 2+ participants");
            return;
        }
        
        let magic: ref<MSNMagicSystem> = MSNMagicSystem.GetInstance();
        if (magic.currentMana >= 300.0) {
            magic.currentMana -= 300.0;
        } else {
            magic.currentMana = 0.0;
        }
        
        let player: ref<PlayerPuppet> = Game.GetPlayer();
        Game.GetStatusEffectSystem().ApplyStatusEffect(player, t"BaseStatusEffect.Regeneration");
        
        this.GrantPerk("MSN_Ritual_MassHeal_Cast");
        this.Notify("Mass Heal ritual complete. Life blooms across the zone.");
    }

    private final func CastDivineAegis(participants: Int) -> Void {
        if (participants < 3) {
            this.Notify("Divine Aegis requires 3+ ritual participants");
            return;
        }
        
        let magic: ref<MSNMagicSystem> = MSNMagicSystem.GetInstance();
        if (magic.currentMana >= 400.0) {
            magic.currentMana -= 400.0;
        } else {
            magic.currentMana = 0.0;
        }
        ...
        this.GrantPerk("MSN_Ritual_DivineAegis_Cast");
        this.Notify("Divine Aegis ritual complete. Absolute protection granted.");
    }
    ...
    public final func StartHellCampaign_InfernalPact() -> Void {
        ...
        this.CorruptSchool(n"Abjuration", n"Flesh_Warping");
        
        this.GrantPerk("MSN_Hell_Infernal_Pact");
        this.Notify("INFERNAL PACT SEALED. Four schools corrupted. Lucifer watches.");
    ...
    private final func GrantCorruptedSpells(school: CName, corruption: CName) -> Void {
        switch (corruption) {
            ...
            case n"Demon_Summoning":
                ...
                break;
            case n"Flesh_Warping":
                this.GrantSpell("Flesh_Armor");
                this.GrantSpell("Blood_Shield");
                this.GrantSpell("Mend_Bone");
                break;
        }
    }
