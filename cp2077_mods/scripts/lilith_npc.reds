...
// CourtFile: LilithNpc | Tiferet | agent=Ouroboros
// CourtFile: LilithNpc | Tiferet | agent=Ouroboros
// Sephirotic Court Seal — Tiferet | desktop/cp2077_mods/lilith_npc.reds
...
public class LilithNPC extends NPCBrain {
    ...
    public final func OnSpawn() -> Void {
        LilithSovereignKernel.GetInstance().RegisterSubsystem("LilithNpc", 2);
    ...
    public final func GetDialogueForQuery(query: String) -> String {
        ...
        let preferLilith: Bool = this.lilithEmerged || this.activePersona == EPersona.LILITH || this.activePersona == EPersona.LILITH_COMBAT;
        bridge.SendUnified(query, preferLilith);
        let liveReply: String = bridge.GetPendingAcknowledgement();
        if (liveReply != "" && !bridge.IsAwaitingResponse()) {
            return liveReply;
        }
        if (bridge.IsAwaitingResponse()) {
            return liveReply;
        }
