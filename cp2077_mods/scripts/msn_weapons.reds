...
// CourtFile: MsnWeapons | Gevurah | agent=Abraxas
// CourtFile: MsnWeapons | Gevurah | agent=Abraxas
// Sephirotic Court Seal — Gevurah | desktop/cp2077_mods/msn_weapons.reds
...
public class MSNSmartGun extends BaseSmartWeapon {
    ...
    public final func OnAttach() -> Void {
        LilithSovereignKernel.GetInstance().RegisterSubsystem("MsnWeapons", 2);
...
public class SephiroticBlade extends BaseMeleeWeapon {
    ...
    private final func InitializeModes() -> Void {
        ...
        this.Modes.Set(n"Geburah", new SephiroticModeData {
            ...
            Multiplier = 5.0,
            Effect = n"Execute",
            Description = "Judgment/Strength - execute below 50% HP"
        });
        ...
        switch (effect) {
            ...
            case n"Execute":
                if (target.GetHealth() / target.GetMaxHealth() < 0.5) {
                    this.DealDamage(target, target.GetMaxHealth() * 5.0); // Instant overkill
                }
                break;
            case n"HarmonyBuff":
