...
// CourtFile: MsnNpcAi | Tiferet | agent=Ouroboros
// CourtFile: MsnNpcAi | Tiferet | agent=Ouroboros
// Sephirotic Court Seal — Tiferet | desktop/cp2077_mods/msn_npc_ai.reds
...
public class MSNNPCBrain extends NPCBrain {
    ...
    public final func OnSpawn() -> Void {
        LilithSovereignKernel.GetInstance().RegisterSubsystem("MsnNpcAi", 2);
    ...
    private final func AdaptStrategic(telemetry: MSNTelemetry) -> Void {
        this.SetCombatStyle(ECombatStyle.Tactical);
        this.SetTargetPriority(ETargetPriority.HighestThreat);
        this.EnablePredictiveBehavior(true);
        this.IncreaseAccuracy(0.1);
    }

    private final func AdaptInsight(telemetry: MSNTelemetry) -> Void {
        this.EnablePredictiveBehavior(true);
        this.IncreaseAccuracy(0.2);
        this.SetTargetPriority(ETargetPriority.WeakestFirst);
    }

    private final func AdaptAnalysis(telemetry: MSNTelemetry) -> Void {
        this.EnableCounterHacking(true);
        this.PrioritizeNetrunningTargets(true);
        this.SetCombatStyle(ECombatStyle.Tactical);
    }

    private final func AdaptBalance(telemetry: MSNTelemetry) -> Void {
        this.SetCombatStyle(ECombatStyle.Balanced);
        this.EnableFlanking(true);
        this.EnableProtectiveBehavior(true);
    }

    public final func OnSpawnComplete() -> Void {
        MSNAIOverhaulController.GetInstance().RegisterNPC(this);
    }
    ...
    private final func UpdateBehaviorWeights(action: String, outcome: Float) -> Void {
        if (outcome > 0.0) {
            this.IncreaseAccuracy(0.02 * outcome);
        } else {
            this.EnableEvasivePatterns(true);
            this.IncreaseMovementSpeed(0.05);
        }
    }
