...
LLAMA_DIR="/home/tehlappy/Desktop/Lilith/llama.cpp"

# Check CUDA toolkit availability
if ! command -v nvcc &>/dev/null; then
    echo "  ⚠️  CUDA Toolkit (nvcc) not found. Install CUDA SDK first:"
    echo "     sudo pacman -S cuda"
    echo "  Falling back to CPU-only build (limited performance)"
    CUDA_SUFFIX=""
else
    CUDA_SUFFIX="-DGGML_CUDA=ON -DLLAMA_CUDA_F16=ON -DLLAMA_CUDA_KVMEM=ON -DLLAMA_CUDA_ASYNC_COPY=ON -DLLAMA_CUDA_KERNEL_FUSION=ON -DLLAMA_CUDA_GRAPH=ON"
fi
...
# draft-model = /home/tehlappy/models/tiny-1b-draft.gguf  # Optional: add draft model for speculative decoding
